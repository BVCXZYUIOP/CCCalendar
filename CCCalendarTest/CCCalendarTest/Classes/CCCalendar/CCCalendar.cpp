...
void CCCalendar::addAGridCell(CCCalendarDayCell *gridCell)
{
    ...
    {
        gridCell->setPosition(ccp(m_fGapX + (m_fGapX + gridCell->getContentSize().width) * m_uFirstDayOfTheWeekThisMonth
                                  , this->getContentSize().height - gridCell->getContentSize().height -  m_fGapY));
    }
    else
    {
...
CCPoint CCCalendar::getLastAvilableCellPosition()
{
    ...
    {
        return ccp (m_fGapX, lastCell->getPosition().y - m_fGapY - lastCell->getContentSize().height);
    }
    else
    {
        return ccp (m_fGapX + lastCell->getPosition().x + lastCell->getContentSize().width, lastCell->getPosition().y);
    }
}
